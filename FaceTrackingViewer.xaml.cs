...
namespace Project_AR
{
    ...
    public partial class FaceTrackingViewer : UserControl, IDisposable
    {
        ...
        {
            if (!Properties.Settings.Default.Facetracking)
                return;
            ...
            internal void OnFrameReady(KinectSensor kinectSensor, ColorImageFormat colorImageFormat, byte[] colorImage, DepthImageFormat depthImageFormat, short[] depthImage, Skeleton skeletonOfInterest)
            {
                ...
                if (this.faceTracker != null)
                {
                    ...
                    if (this.lastFaceTrackSucceeded)
                    {
                        ...
                        for (int i = 1; i < 8; i++)
                        {
                            p1.X += facePoints3D[i].X;
                            p1.Y += facePoints3D[i].Y;
                            p1.Z += facePoints3D[i].Z;

                            p2.X += facePoints3D[8 + i].X;
                            p2.Y += facePoints3D[8 + i].Y;
                            p2.Z += facePoints3D[8 + i].Z;
                        ...
                        p3.X = (facePoints3D[76].X + facePoints3D[77].X) / 2;
                        p3.Y = (facePoints3D[76].Y + facePoints3D[77].Y) / 2;
                        p3.Z = (facePoints3D[76].Z + facePoints3D[77].Z) / 2;
                        ...
                        //cross product of V2 X V1
                        float ox = v1y * v2z - v2y * v1z;
                        float oy = v1z * v2x - v2z * v1x;
                        float oz = v1x * v2y - v2x * v1y;

                        float normalize = (float)Math.Sqrt(ox * ox + oy * oy + oz * oz);
                        nx = ox / normalize;
                        ny = oy / normalize;
                        nz = oz / normalize;
